...
class Pocket_Generator:
    def __init__(self, parent_knitgraph: Knit_Graph, old_carrier_id:int, new_carrier_id: int, left_keynodes_child_fabric, right_keynodes_child_fabric, close_top: bool, edge_connection_left_side: List[bool], edge_connection_right_side: List[bool], parent_graph_course_to_loop_ids: Optional[Dict[int, List[int]]] = None, parent_graph_node_to_course_and_wale: Optional[Dict[int, Tuple[int, int]]] = None, parent_graph_course_and_wale_to_node: Optional[Dict[Tuple[int, int], int]] = None, parent_graph_is_polygon: bool = False, child_graph_is_polygon: bool = True):
        """
        ...
        self.child_graph_node_on_front_or_back: Dict[int, str] = {}
        #
        ...
        if parent_graph_is_polygon == False: #then the input knit graph is a common retangle, it is safe to invoke knitgraph.get_course() to get correct info.
            self.parent_graph_loop_id_to_course, self.parent_graph_course_to_loop_ids, self.parent_graph_loop_id_to_wale, self.parent_graph_wale_to_loop_ids = self.parent_knitgraph.get_courses(unmodified=True)
    ...
    def generate_polygon_from_keynodes(self):
        ...
        for node in node_to_course_and_wale.keys():
            ...
            self.child_graph_node_on_front_or_back[loop_id] = 'f'
        ...
        KnitGraph_Visualizer = knitGraph_visualizer(knit_graph = self.child_knitgraph, node_on_front_or_back = self.child_graph_node_on_front_or_back, node_to_course_and_wale = node_to_course_and_wale, object_type = 'sheet')
    ...
    def build_rows_on_parent_graph_just_above_splitting_course_id(self):
        ...
        stop_course_id = [*self.child_graph_course_to_loop_ids.keys()][0]
        for course_id in range(start_course_id, stop_course_id + 1):
            for i in range(len(self.parent_graph_course_id_to_wale_ids[course_id])):
                ...
                self.parent_graph_course_and_wale_to_node[(course_id, wale_id)] = loop_id
    ...
    def grow_one_row(self, course_id, on_parent_graph: bool):
        if on_parent_graph == True:
            ...
            for i in range(len(self.child_graph_course_id_to_wale_ids[course_id])):
                ...
                self.child_graph_course_and_wale_to_node[(course_id, wale_id)] = loop_id
                self.child_graph_course_to_loop_ids[course_id].append(loop_id)
                self.node_on_front_or_back[loop_id] = 'b'
            #connect bottom row of child fabric to splitting row
            # print(f'self.child_graph_course_and_wale_to_node is {self.child_graph_course_and_wale_to_node}')
            if course_id == [*self.child_graph_course_to_loop_ids.keys()][0]:
                for wale_id in range(self.left_keynodes_child_fabric[0][1], self.right_keynodes_child_fabric[0][1]+1):
                    parent_loop_id = self.parent_graph_course_and_wale_to_node[(course_id-1, wale_id)]
                    loop_id = self.child_graph_course_and_wale_to_node[(course_id, wale_id)]
    ...
    def get_nodes_on_each_edge_on_child_fabric(self):
        ...
        for i in range(1, num_of_nodes_left_side):
            ...
            for course_id in range(last_course_id, curr_course_id+1):
                if course_id % 2 == 0:
                    smaller_wale_edge_nodes = self.child_graph_course_to_loop_ids[course_id][0]
                elif course_id % 2 == 1:
                    smaller_wale_edge_nodes = self.child_graph_course_to_loop_ids[course_id][-1]
        ...
        for i in range(1, num_of_nodes_right_side):
            ...
            for course_id in range(last_course_id, curr_course_id+1):
                if course_id % 2 == 0:
                    bigger_wale_edge_nodes = self.child_graph_course_to_loop_ids[course_id][-1]
                elif course_id % 2 == 1:
                    bigger_wale_edge_nodes = self.child_graph_course_to_loop_ids[course_id][0]
    ...
    def get_target_edge_nodes_on_parent_fabric(self, edge_nodes_smaller_wale_side_edges_child, edge_nodes_bigger_wale_side_edges_child):
        ...
        for edge_index in edge_nodes_bigger_wale_side_edges_child.keys():
            ...
            for edge_node in edge_nodes:
                course_id = self.child_graph_node_to_course_and_wale[edge_node][0]
                wale_id = self.child_graph_node_to_course_and_wale[edge_node][1]
                edge_nodes_bigger_wale_side_edges_parent[edge_index].append(self.parent_graph_course_and_wale_to_node[(course_id, wale_id)])
        for edge_index in edge_nodes_smaller_wale_side_edges_child.keys():
            ...
            for edge_node in edge_nodes:
                course_id = self.child_graph_node_to_course_and_wale[edge_node][0]
                wale_id = self.child_graph_node_to_course_and_wale[edge_node][1]
                edge_nodes_smaller_wale_side_edges_parent[edge_index].append(self.parent_graph_course_and_wale_to_node[(course_id, wale_id)])
        ...
        for edge_index in range(num_of_left_edges):
            ...
            if connection_property == True:
                for node in edge_nodes_smaller_wale_side_edges_parent[edge_index][:-1]:
                    ...
                    target_node_on_child_fabric = self.child_graph_course_and_wale_to_node[(course_id+1, wale_id)]
                    if (course_id, wale_id) in self.parent_graph_course_and_wale_to_node:
                        edge_split_node = self.parent_graph_course_and_wale_to_node[(course_id, wale_id)]
                        self._knit_graph.connect_loops(edge_split_node, target_node_on_child_fabric, pull_direction = Pull_Direction.FtB, parent_offset =  0)
    ...
    def build_pocket_graph(self) -> Knit_Graph:   
        ...
        KnitGraph_Visualizer = knitGraph_visualizer(knit_graph = self._knit_graph, node_on_front_or_back = self.node_on_front_or_back, node_to_course_and_wale = self.pocket_graph_node_to_course_and_wale, object_type = 'pocket')
        KnitGraph_Visualizer.visualize()
