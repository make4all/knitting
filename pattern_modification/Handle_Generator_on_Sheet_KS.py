...
class Handle_Generator_on_Sheet:
    def __init__(self, parent_knitgraph: Knit_Graph, sheet_yarn_carrier_id: int, handle_yarn_carrier_id: int, is_front_patch: bool, left_keynodes_child_fabric, right_keynodes_child_fabric):
        ...
        self.sheet_yarn: Yarn = Yarn("sheet_yarn", self.handle_graph, carrier_id=self.sheet_yarn_carrier_id)
    ...
    def grow_one_row(self, course_id, on_parent_graph: bool):
        if on_parent_graph == True:
            ...
            # because for handle we only need to connect two edge bottom node of child fabric to two splitting edge nodes
            ...
            if course_id == [*self.child_knitgraph.course_to_loop_ids.keys()][0]:
                for wale_id in range(self.left_keynodes_child_fabric[0][1], self.right_keynodes_child_fabric[0][1]+self.wale_dist, self.wale_dist):
                    if wale_id == self.left_keynodes_child_fabric[0][1] or wale_id == self.right_keynodes_child_fabric[0][1]:
                        ...
                        self.handle_graph.connect_loops(parent_loop_id, loop_id, pull_direction = Pull_Direction.BtF, parent_offset = 1)
        ...
        for edge_index in range(num_of_left_edges):
            for node in edge_nodes_smaller_wale_side_edges_parent[edge_index][1:]:
                ...
                if (course_id, wale_id) in self.parent_knitgraph_course_and_wale_to_node:
                    ...
                    self.handle_graph.connect_loops(edge_split_node, target_node_on_child_fabric, pull_direction = Pull_Direction.BtF, parent_offset = 1)
