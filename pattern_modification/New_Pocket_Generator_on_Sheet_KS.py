...
class Pocket_Generator_on_Sheet:
    def __init__(self, parent_knitgraph: Knit_Graph, sheet_yarn_carrier_id: int, pocket_yarn_carrier_id: int, is_front_patch: bool, left_keynodes_child_fabric, right_keynodes_child_fabric, close_top: bool, edge_connection_left_side: List[bool], edge_connection_right_side: List[bool]):
        """
        ...
        self.sheet_yarn: Yarn = Yarn("sheet_yarn", self.pocket_graph, carrier_id=self.sheet_yarn_carrier_id)
        ...
        if on_parent_graph == True:
            ...
            if course_id == [*self.child_knitgraph.course_to_loop_ids.keys()][0]:
                for wale_id in range(self.left_keynodes_child_fabric[0][1], self.right_keynodes_child_fabric[0][1]+self.wale_dist, self.wale_dist):
                    parent_loop_id = self.parent_knitgraph_course_and_wale_to_node[(course_id-1, wale_id+1)]
                    ...
                    self.pocket_graph.connect_loops(parent_loop_id, loop_id, pull_direction = Pull_Direction.BtF, parent_offset = 1)
    ...
    def get_target_edge_nodes_on_parent_fabric(self, edge_nodes_smaller_wale_side_edges_child, edge_nodes_bigger_wale_side_edges_child):
        ...
        for edge_index in edge_nodes_bigger_wale_side_edges_child.keys():
            ...
            for edge_node in edge_nodes:
                ...
                edge_nodes_bigger_wale_side_edges_parent[edge_index].append(self.parent_knitgraph_course_and_wale_to_node[(course_id-1, wale_id+1)])
        for edge_index in edge_nodes_smaller_wale_side_edges_child.keys():
            ...
            for edge_node in edge_nodes:
                course_id = self.child_knitgraph.node_to_course_and_wale[edge_node][0]
                wale_id = self.child_knitgraph.node_to_course_and_wale[edge_node][1]
                edge_nodes_smaller_wale_side_edges_parent[edge_index].append(self.parent_knitgraph_course_and_wale_to_node[(course_id-1, wale_id+1)])
        ...
        for edge_index in range(num_of_left_edges):
            ...
            if connection_property == True:
                for node in edge_nodes_smaller_wale_side_edges_parent[edge_index][1:]:
                    ...
                    target_node_on_child_fabric = self.child_knitgraph_course_and_wale_to_node[(course_id+1, wale_id-1)]
                    if (course_id, wale_id) in self.parent_knitgraph_course_and_wale_to_node:
                        edge_split_node = self.parent_knitgraph_course_and_wale_to_node[(course_id, wale_id)]
                        self.pocket_graph.connect_loops(edge_split_node, target_node_on_child_fabric, pull_direction = Pull_Direction.BtF, parent_offset = 1)
    ...
    def build_pocket_graph(self) -> Knit_Graph:   
        ...
        if self.close_top == True:
            for node in self.child_knitgraph.course_to_loop_ids[last_course_id_child_fabric]:
                ...
                node_to_connect = self.parent_knitgraph_course_and_wale_to_node[(course+1, wale+1)] 
        ...
        # KnitGraph_Visualizer = knitGraph_visualizer(knit_graph = self.pocket_graph, object_type = 'pocket')
        # KnitGraph_Visualizer.visualize()
