...
class Pocket_Generator:
    ...
    def check_keynodes_validity(self): 
        ...
        if self.spliting_starting_course_id % 2 == 0:
            ...
            assert first_keynode_right[1] == self.parent_graph_node_to_course_and_wale[self.sorted_spliting_nodes[0]][1], \
                f'wale id of last keynode on the left: {first_keynode_right[1]} does not match the wale id of the splitting node: {self.parent_graph_node_to_course_and_wale[self.sorted_spliting_nodes[-1]][1]}'
    ...
    def grow_one_row_new(self, course_id, on_parent_graph: bool):
    ...
    def get_edge_nodes_on_child_fabric(self):
    ...
    def get_target_edge_nodes_on_parent_fabric(self, bigger_wale_edge_nodes, smaller_wale_edge_nodes):
    ...
    # @deprecated("Deprecated because this is incorrect knitgraph representation of a pocket")
    ...
    def build_pocket_graph(self, close_top: bool = False, smaller_wale_edge_connected: bool = False, bigger_wale_edge_connected: bool = False) -> Knit_Graph:   
        ...
        self.grow_one_row_new(course_id = [*self.child_graph_course_to_loop_ids.keys()][0], on_parent_graph = False)
        for course_id in [*self.child_graph_course_to_loop_ids.keys()][1:]:
            self.grow_one_row_new(course_id, on_parent_graph = True)
            self.grow_one_row_new(course_id, on_parent_graph = False)
        ...
        smaller_wale_edge_nodes, bigger_wale_edge_nodes = self.get_edge_nodes_on_child_fabric()
        smaller_wale_target_edge_nodes, bigger_wale_target_edge_nodes = self.get_target_edge_nodes_on_parent_fabric(bigger_wale_edge_nodes, smaller_wale_edge_nodes)
        self.connect_stitches_on_knitgraph()
        self.actions_around_target_edge_node(smaller_wale_target_edge_nodes, bigger_wale_target_edge_nodes, smaller_wale_edge_connected, bigger_wale_edge_connected)
...
if __name__ == '__main__':
    ...
    # generator.build_pocket_graph(close_top=False, smaller_wale_edge_connected=True, bigger_wale_edge_connected = True)
    #2
    # polygon_generator = Polygon_Generator(left_keynodes_child_fabric = [(0, 0), (9, 0)], right_keynodes_child_fabric = [(0, 3), (3, 6), (6, 6), (9, 3)])
    # parent_knitgraph = polygon_generator.generate_polygon_from_keynodes(yarn_id = 'yarn', carrier_id = 3)
    ...
    generator.build_pocket_graph(close_top=True, smaller_wale_edge_connected=True, bigger_wale_edge_connected = True)
