...
class Pocket_Generator_on_Tube:
    def __init__(self, parent_knitgraph: Knit_Graph, tube_yarn_carrier_id:int, pocket_yarn_carrier_id: int, is_front_patch: bool, left_keynodes_child_fabric, right_keynodes_child_fabric, close_top: bool, edge_connection_left_side: List[bool], edge_connection_right_side: List[bool]):
        ...
        self.tube_yarn: Yarn = Yarn("tube_yarn", self.pocket_graph, carrier_id=self.tube_yarn_carrier_id)
        ...
        if on_parent_graph == True:
            ...
            if course_id == [*self.child_knitgraph.course_to_loop_ids.keys()][0]:
                for wale_id in range(self.left_keynodes_child_fabric[0][1], self.right_keynodes_child_fabric[0][1]+self.wale_dist, self.wale_dist):
                    ...
                    self.pocket_graph.connect_loops(parent_loop_id, loop_id, pull_direction = Pull_Direction.BtF, parent_offset = 1 if self.is_front_patch == False else -1)
                    # self.pocket_graph.connect_loops(parent_loop_id, loop_id, pull_direction = Pull_Direction.FtB if self.is_front_patch == False else Pull_Direction.BtF, parent_offset = 1 if self.is_front_patch == False else -1)
