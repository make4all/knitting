            ...
            def preprocessing_on_graph(G):
                ...
                for node in G1.nodes:
                    ...
                    if course_id < max([*self._course_to_loop_ids.keys()]):
                        ...
                        #         right_above_node = self.course_and_wale_to_node[(course_id+1, wale_id+1)]
                        #         G1.add_edge(node, right_above_node, weight = -1)
                        # updated as below for node in between on each course, because above method does not work for decreased tube, specifically, the last node on the front
                        ...
                        # for node in beweetn on each course 
                            ...
                            if (course_id+1, wale_of_smaller_nearest_neighbor) in self.course_and_wale_to_node.values():
                            ...
                            if (course_id+1, wale_of_bigger_nearest_neighbor) in self.course_and_wale_to_node.values():
