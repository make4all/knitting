...
class Hole_Generator_on_Sheet:
    ...
    """
    def __init__(self, yarns_and_holes_to_add: Dict[int, List[int]], polygon_generator: Optional[Polygon_Generator] = None, simple_knitgraph_generator: Optional[Simple_Knitgraph_Generator] = None):
        if polygon_generator != None:
            self.is_polygon: bool = True
            self._knit_graph: Knit_Graph = polygon_generator.generate_polygon_from_keynodes(yarn_id = 'yarn', carrier_id = 3)
            self.starting_nodes_coor: List[Tuple[int, int]] = polygon_generator.starting_nodes_coor
            assert self.starting_nodes_coor is not None
            self.ending_nodes_coor: List[Tuple[int, int]] = polygon_generator.ending_nodes_coor
            assert self.ending_nodes_coor is not None
            self._course_to_loop_ids: Dict[float, List[int]]  = polygon_generator.course_to_loop_ids 
            self.node_to_course_and_wale: Dict[int, Tuple(int, int)] = polygon_generator.node_to_course_and_wale
            self.course_and_wale_to_node: Dict[Tuple[int, int], int]  =  polygon_generator.course_and_wale_to_node
            self.node_on_front_or_back: Dict[int, str] = polygon_generator.node_on_front_or_back
            self.gauge = polygon_generator.gauge
        elif simple_knitgraph_generator != None:
            ...
            self.course_and_wale_to_node = simple_knitgraph_generator.course_and_wale_to_node
        ...
        assert self._course_to_loop_ids is not None
        assert self.node_to_course_and_wale is not None
        assert self.course_and_wale_to_node is not None
    ...
    def remove_hole_nodes_from_graph(self):
        ...
        self.course_and_wale_to_node = {v: k for k, v in self.node_to_course_and_wale.items()}
    ...
    def get_start_and_end_wale_id_per_course(self):
        ...
        for course_id in self._course_to_loop_ids.keys():
            if course_id % 2 == 0:
                ...
                course_id_to_start_wale_id[course_id] = start_wale_id
                course_id_to_end_wale_id[course_id] = end_wale_id                 
        # if the more generalizable solution is required in the future as the project develop, use self._course_to_loop_ids and self._loop_id_to_wale to solve.
        ...
        for hole_index in [*self.holes_size.keys()]:
            ...
            if hole_info['hole_start_course'] % 2 == 0:
                ...
                for course_id in range(hole_info['hole_start_course'], hole_info['hole_end_course'] + 1):
                    ...
                    if (course_id, smallest_wale_id-(self.float_length+1)) not in self.course_and_wale_to_node.keys():
                    ...
                    if (course_id, biggest_wale_id + (self.float_length+1)) not in self.course_and_wale_to_node.keys():
            ...
            if hole_info['hole_start_course'] % 2 == 1:
                ...
                for course_id in range(hole_info['hole_start_course'], hole_info['hole_end_course'] + 1):
                    ...
                    biggest_wale_id = max(self._hole_course_to_wale_ids[hole_index][course_id])
                    if (course_id, biggest_wale_id + (self.float_length+1)) not in self.course_and_wale_to_node.keys():
                        print(f'no node is found on the hole margin near the old yarn side {(course_id, biggest_wale_id + (self.float_length+1))}')
                    else:
                        old_yarn_course_to_margin_loop_ids[course_id] = self.course_and_wale_to_node[(course_id, biggest_wale_id + (self.float_length+1))]
                    if  (course_id, smallest_wale_id - (self.float_length+1)) not in self.course_and_wale_to_node.keys():
                        print(f'no node is found on the hole margin near the new yarn side {(course_id, smallest_wale_id - (self.float_length+1))}')
                    else:
                        new_yarn_course_to_margin_loop_ids[course_id] = self.course_and_wale_to_node[(course_id, smallest_wale_id - (self.float_length+1))]
                    new_yarn_course_to_loop_ids[course_id] = []
                    if course_id % 2 == 0:
    ...
    def add_hole(self):
        ...
        KnitGraph_Visualizer = knitGraph_visualizer(self._knit_graph, node_on_front_or_back = self.node_on_front_or_back, node_to_course_and_wale = self.node_to_course_and_wale, object_type = 'sheet')
