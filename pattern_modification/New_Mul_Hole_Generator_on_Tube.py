...
class Hole_Generator_on_Tube:
    ...
    """
    def __init__(self, hole_index_to_holes: Dict[int, List[int]], simple_knitgraph_generator: Simple_Knitgraph_Generator = None):
        ...
        self.node_to_course_and_wale =  simple_knitgraph_generator.node_to_course_and_wale
        self.node_on_front_or_back = simple_knitgraph_generator.node_on_front_or_back
        self.course_and_wale_and_bed_to_node: Dict[((int, int), str), int] = simple_knitgraph_generator.course_and_wale_and_bed_to_node
        # reverse to retrieve bed info, which would be used in rebuild_yarn_graph()
        self.node_to_course_and_wale_and_bed: Dict[int, ((int, int), str)] = {v: k for k, v in self.course_and_wale_and_bed_to_node.items()}
        self._loop_id_to_course: Dict[int, float]
        self._loop_id_to_course, self._course_to_loop_ids = self._knit_graph.get_courses()
        self.gauge = simple_knitgraph_generator.gauge
        self.float_length = int(1/self.gauge) - 1
    ...
    def get_course_id_to_wale_ids(self):
        for course_id in self._course_to_loop_ids.keys():
            ...
            for loop_id in self._course_to_loop_ids[course_id]:
                wale_id = self.node_to_course_and_wale[loop_id][1]
    ...
    def preprocessing_on_graph(self, G1):
        ...
        for course_id in self._course_to_loop_ids.keys():
            ...
            if abs(self.node_to_course_and_wale[start_node][1] - self.node_to_course_and_wale[end_node][1]) <= self.float_length+1:
        ...
        for node in G1.nodes:
            ...
            bed = self.node_on_front_or_back[node]
            ...
            if course_id < max([*self._course_to_loop_ids.keys()]):
                ...
                if wale_id == min_wale_id_on_the_course:
                    #node right above it
                    if ((course_id+1, wale_id), bed) in self.course_and_wale_and_bed_to_node.keys():
                        above_node = self.course_and_wale_and_bed_to_node[((course_id+1, wale_id), bed)]
                    ...
                    if ((course_id+1, wale_of_bigger_nearest_neighbor), bed) in self.course_and_wale_and_bed_to_node.keys():
                        big_wale_above_node = self.course_and_wale_and_bed_to_node[((course_id+1, wale_of_bigger_nearest_neighbor), bed)]
                    ...
                    for wale_id_above_course in wale_ids_on_above_course:
                        ...
                        if wale_difference >= 0 and wale_difference < min_wale_difference: #>= instead of > is because for consistent tube it is the same wale
                            min_wale_difference = wale_difference
                            wale_of_bigger_nearest_neighbor = wale_id_above_course
                    if ((course_id+1, wale_of_bigger_nearest_neighbor), opposite_bed) in self.course_and_wale_and_bed_to_node.keys():
                        big_wale_above_node = self.course_and_wale_and_bed_to_node[((course_id+1, wale_of_bigger_nearest_neighbor), opposite_bed)]
                        G1.add_edge(node, big_wale_above_node, weight = -1)
                # for node in between on each course 
                    ...
                    if min_wale_difference == self.float_length+1 and ((course_id+1, wale_of_smaller_nearest_neighbor), bed) in self.course_and_wale_and_bed_to_node.keys():
                        small_wale_above_node = self.course_and_wale_and_bed_to_node[((course_id+1, wale_of_smaller_nearest_neighbor), bed)]
                    ...
                    for wale_id_above_course in wale_ids_on_above_course:
                        ...
                        if wale_difference > 0 and wale_difference < min_wale_difference: 
                            min_wale_difference = wale_difference
                            wale_of_bigger_nearest_neighbor = wale_id_above_course
                    if min_wale_difference == self.float_length+1 and ((course_id+1, wale_of_bigger_nearest_neighbor), bed) in self.course_and_wale_and_bed_to_node.keys():
                        big_wale_above_node = self.course_and_wale_and_bed_to_node[((course_id+1, wale_of_bigger_nearest_neighbor), bed)]
                        G1.add_edge(node, big_wale_above_node, weight = -1) 
                        # if node == 76:
                    ...
                    for wale_id_above_course in wale_ids_on_above_course:
                        ...
                        if wale_difference > 0 and wale_difference < min_wale_difference:
                            min_wale_difference = wale_difference
                            wale_of_smaller_nearest_neighbor = wale_id_above_course
                    if ((course_id+1, wale_of_smaller_nearest_neighbor), bed) in self.course_and_wale_and_bed_to_node.keys():
                        small_wale_above_node = self.course_and_wale_and_bed_to_node[((course_id+1, wale_of_smaller_nearest_neighbor), bed)]
                        G1.add_edge(node, small_wale_above_node, weight = -1)
                    ...
                    for wale_id_above_course in wale_ids_on_above_course:
                        ...
                        if wale_difference >= 0 and wale_difference < min_wale_difference: #>= instead of > is because for consistent tube it is the same wale
                            min_wale_difference = wale_difference
                            wale_of_smaller_nearest_neighbor = wale_id_above_course
                    if ((course_id+1, wale_of_smaller_nearest_neighbor), opposite_bed) in self.course_and_wale_and_bed_to_node.keys():
                        small_wale_above_node = self.course_and_wale_and_bed_to_node[((course_id+1, wale_of_smaller_nearest_neighbor), opposite_bed)]
                        G1.add_edge(node, small_wale_above_node, weight = -1)
    ...
    def remove_hole_nodes_from_graph(self):
        ...
        for hole in self.hole_index_to_holes.values():
            ...
            for hole_node in hole:
                del self.node_to_course_and_wale[hole_node]
                del self.node_on_front_or_back[hole_node] 
                del self.node_to_course_and_wale_and_bed[hole_node]
        # then use the updated self.node_to_course_and_wale_and_bed to get updated self.course_and_wale_and_bed_to_node
        self.course_and_wale_and_bed_to_node = {v: k for k, v in self.node_to_course_and_wale_and_bed.items()}

    def get_start_and_end_wale_id_per_course(self):
        ''' 
        abandoned because unlike sheet, for tube, the start_node for each course is always self._course_to_loop_ids[course_id][0] 
    ...
    def connect_hole_edge_nodes(self):
        for hole_index in [*self.holes_size.keys()]:
            if len(self._hole_course_to_wale_ids[hole_index].keys()) > 1:
                ...
                hole_edge_node_small_wale_id =  min(self._hole_course_to_wale_ids[hole_index][hole_start_course]) - (self.float_length+1)
                hole_edge_node_big_wale_id = max(self._hole_course_to_wale_ids[hole_index][hole_start_course]) + (self.float_length+1)
                # for hole_edge_node_small_wale and hole_edge_node_big_wale, the connected node should be 
                connected_node_small_wale_id_for_hole_edge = min(self._hole_course_to_wale_ids[hole_index][hole_start_course+1]) - (self.float_length+1)
                connected_node_big_wale_id_for_hole_edge = max(self._hole_course_to_wale_ids[hole_index][hole_start_course+1]) + (self.float_length+1)
    ...
    def bind_off(self):
        for hole_index in [*self.holes_size.keys()]:
            ...
            bottom_nodes = self._course_to_loop_ids[hole_bottom_course_id]
    ...
    def read_connectivity_from_knitgraph(self):
        ...
        for edge_data in knitgraph_edge_data_list:
            ...
            node_bed = self.node_on_front_or_back[node]
            predecessor = edge_data[0]
            predecessor_coor = self.node_to_course_and_wale[predecessor]
            predecessor_bed = self.node_on_front_or_back[predecessor]
            ...
            if len(yarn.yarn_graph.nodes) > 0:
                ...
                for node in nodes_on_yarn:
                    ...
                    if course_id not in course_to_wale_ids:
                        ...
                        course_to_bed_info[course_id] = [self.node_to_course_and_wale_and_bed[node][1]]
                    else:
                        course_to_wale_ids[course_id].append(self.node_to_course_and_wale[node][1])
                        course_to_bed_info[course_id].append(self.node_to_course_and_wale_and_bed[node][1])
                ...
                for course_id in sorted([*course_to_wale_ids.keys()]):
                    ...
                    for wale_id, bed in zip(wale_ids, bed_info):
                        ...
                        self.node_on_front_or_back[loop_id] = bed
                        self.course_and_wale_and_bed_to_node[((course_id, wale_id), bed)] = loop_id
                        self.node_to_course_and_wale_and_bed[loop_id] = ((course_id, wale_id), bed)
                        i+=1
    ...
    def connect_stitches_on_knitgraph(self):
        ...
        for (parent_coor, parent_bed, child_coor, child_bed, attr_dict) in self.knitgraph_coors_connectivity:
            parent_node = self.course_and_wale_and_bed_to_node[(parent_coor, parent_bed)]
            child_node = self.course_and_wale_and_bed_to_node[(child_coor, child_bed)]
    ...
    def add_hole(self):
        ...
        KnitGraph_Visualizer = knitGraph_visualizer(self._knit_graph, node_on_front_or_back = self.node_on_front_or_back, node_to_course_and_wale = self.node_to_course_and_wale, object_type = 'tube')
