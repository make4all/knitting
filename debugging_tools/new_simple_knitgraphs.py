...
def decrease_tube(width: int = 6, carrier:int=3) -> Knit_Graph:
    """
    ...
    for _ in range(height):
        ...
        for parent_id in (non_edge_parent):
            ...
            if parent_id == prior_row[1]:
                knitGraph.connect_loops(parent_id, child_id, pull_direction = pull_direction)
                knitGraph.connect_loops(prior_row[0], child_id, parent_offset = -1, pull_direction = pull_direction)
            elif parent_id == prior_row[course_width+1]:
                knitGraph.connect_loops(parent_id, child_id, pull_direction = pull_direction)
                knitGraph.connect_loops(prior_row[course_width], child_id, parent_offset = -1, pull_direction = pull_direction)
            elif parent_id == prior_row[course_width-2]:
                knitGraph.connect_loops(parent_id, child_id, pull_direction = pull_direction)
                knitGraph.connect_loops(prior_row[course_width-1], child_id, parent_offset = 1, pull_direction = pull_direction)
            elif parent_id == prior_row[-2]:
                knitGraph.connect_loops(parent_id, child_id, pull_direction = pull_direction)
                knitGraph.connect_loops(prior_row[-1], child_id, parent_offset = 1, pull_direction = pull_direction)
            else:
                knitGraph.connect_loops(parent_id, child_id, pull_direction = pull_direction)
        prior_row = next_row
...
def lace1(width: int = 4, height: int = 4):
    """
    :param width: the number of stitches of the swatch
    :param height:  the number of courses of the swatch
    :return: a knitgraph with k2togs and yarn-overs surrounded by knit wales
    ...
    for row in range(1, height):
        ...
        reversed_prior_row = [*reversed(prior_row)]
        for col, parent_id in enumerate(reversed_prior_row):
            if row % 2 == 0 or col % 9 == 2 or col % 9 ==3 or col % 9 ==4 or col % 9 ==5 or col % 9 ==6 or col % 9 ==0 or col % 9 ==1:  # knit on even rows and before and after twists
            # if row % 2 == 0 or col % 6 == 0 or col % 6 == 3 or col % 6 == 4 or col % 6 == 5:  # knit on even rows and before and after twists
                add_loop_and_knit(parent_id)
            elif col % 9 == 7:
                child_id, child = yarn.add_loop_to_end()
                knitGraph.add_loop(child)
