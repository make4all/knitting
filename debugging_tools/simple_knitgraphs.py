...
def test_parentsoffset(width: int = 4, height: int = 2, carrier:int=3) -> Knit_Graph:
    """
    :param carrier:
    :param width: the number of stitches of the swatch
    :param height:  the number of courses of the swatch
    ...
    for _ in range(1, height):
        ...
        for parent_id in reversed(prior_row):
            ...
            next_row.append(child_id)
            knitGraph.add_loop(child)
            if _ == 1 and parent_id == 1:
                # print('1', child_id)
                knitGraph.connect_loops(parent_id, child_id, parent_offset = -1, depth = 1)
            elif _ == 1 and parent_id == 2:
                # print('2', child_id)
                knitGraph.connect_loops(parent_id, child_id, parent_offset = 1, depth = -1)
            else:
                knitGraph.connect_loops(parent_id, child_id)
...
#round_stockinette_1
def round_stockinette1(width: int = 4, height: int = 4, carrier:int=3) -> Knit_Graph:
    """
    :param carrier:
    :param width: the number of stitches of the swatch
    :param height:  the number of courses of the swatch
    ...
    for _ in range(0, width):
        ...
        first_row.append(loop_id)
        knitGraph.add_loop(loop)
    # yarn.connect_end_to_start(width)
...
# round_stockinette_2
def round_stockinette2(width: int = 4, height: int = 4, carrier:int=3) -> Knit_Graph:
    """
    :param carrier:
    :param width: the number of stitches of the swatch
    :param height:  the number of courses of the swatch
    ...
    first_row = []
    for _ in range(0, width):
        loop_id, loop = yarn.add_loop_to_end(width, round = True)
        first_row.append(loop_id)
        knitGraph.add_loop(loop)
    # yarn.connect_end_to_start(width)
    ...
    for _ in range(1, height):
        next_row = []
        for parent_id in prior_row:
            child_id, child = yarn.add_loop_to_end(width, round = True)
            next_row.append(child_id)
            knitGraph.add_loop(child)
            knitGraph.connect_loops(parent_id, child_id)
        prior_row = next_row
...
def stockinette_with_hole(hole_position: List[int], hole_width: int = 1, width: int = 4, height=5) -> Knit_Graph:
    #add hole through decrease & increase

    #hole must be occured in locations other than on border
    #hole position (only leftmost location is sufficient) and its width, height of hole is always considered to be 
    #1 here, will update in the future.
    hole_course_index = hole_position[0]
    hole_wale_index = hole_position[1]
    assert 0 < hole_course_index < height - 1 and 0 < hole_wale_index < width - 1 and hole_wale_index + (hole_width - 1) < width - 1, \
        "hole position is invalid"

    #convert wale_index to col_index that we actually use to locate hole in our program
    hole_col_index = ((width - 1) - hole_wale_index) if hole_course_index % 2 == 1 else hole_wale_index
    print('hole_col_index',  hole_col_index)
    #establish first row
    knitGraph = Knit_Graph()
    yarn = Yarn("yarn", knitGraph)
    knitGraph.add_yarn(yarn)
    ...
    for course in range(1, height):
        ...
        #courses that apply add_loop_and_knit() all the way down
        if not course == hole_course_index and not course == hole_course_index + 1 :
            for col, parent_id in enumerate(reversed_prior_course):            
                add_loop_and_knit(parent_id)
                
        #courses that need attention
        elif course == hole_course_index:
            for col in range(width):
                if col in range(hole_col_index, hole_col_index + hole_width):
                    continue
                else:
                    if hole_width == 1:
                        if col == hole_col_index - 1:
                            depth = 1
                        else: 
                            depth = 0
                        add_loop_and_knit(reversed_prior_course[col], depth)
                        if col == hole_col_index - 1:
                            knitGraph.connect_loops(reversed_prior_course[col+1], knitGraph.last_loop_id, depth=-1,\
                                                    parent_offset = 1)
                    if hole_width > 1:
                        if col == hole_col_index - 1 or col == hole_col_index + hole_width:
                            depth = 1
                        else: 
                            depth = 0
                        add_loop_and_knit(reversed_prior_course[col], depth)
                        if col == hole_col_index - 1:
                            knitGraph.connect_loops(reversed_prior_course[col+1], knitGraph.last_loop_id, depth=-1,\
                                                    parent_offset = 1)
                        if col == hole_col_index + hole_width :
                            knitGraph.connect_loops(reversed_prior_course[col-1], knitGraph.last_loop_id, depth=-1,\
                                                parent_offset = 0)
                    
        elif course == hole_course_index + 1:
            if course % 2 == 1:
                increase_index = width - (hole_col_index + hole_width)
            for col in range(width):
                child_id, child = yarn.add_loop_to_end()
                next_course.append(child_id)
                knitGraph.add_loop(child)
                if not col in range(increase_index, increase_index + hole_width):
                    knitGraph.connect_loops(reversed_prior_course.pop(0), knitGraph.last_loop_id)
                    print('knitGraph.last_loop_id', knitGraph.last_loop_id)
...
def tube_with_hole_by_short_row(hole_position: List[int], hole_width: int = 1, hole_height: int = 1, width: int = 5, height: int = 5) -> Knit_Graph:
    # hole_position: List[int], i.e., [course_index, wale_index]
    """
    :param buffer_height: THe height of the buffer on top and bottom
    :param width: the width of the swatch, must be greater than 4
