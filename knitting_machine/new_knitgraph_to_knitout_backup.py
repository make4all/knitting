...
class Knitout_Generator:
    ...
    def __init__(self, knit_graph: Knit_Graph, object_type:str = 'sheet', node_to_course_and_wale:Optional[Dict[int, Tuple[int,int]]] = None, unmodified: bool = True):
        """
        ...
        loop_id_to_course, courses_to_loop_ids, loop_ids_to_wale, wale_to_loop_ids = self._knit_graph.get_courses(unmodified)
        print(f'courses_to_loop_ids in knitgraph to knitout is {courses_to_loop_ids} ')
        if unmodified==False:
            assert node_to_course_and_wale != None, f"node_to_course_and_wale is needed"
        ...
        if node_to_course_and_wale is not None:
        ...
        # used in Carriage Pass, not necessary for sheet case, but required for tube case because it involves walking the yarn across back and front bed
        self.loop_id_to_needle_pos: Dict[int, int] = {}
    ...
    #@deprecated get_first_course_id_of_yarns_in
    def deprecated_get_first_course_id_of_yarns_in(self):
        # 1
    ...
    def _knit_and_split_row(self, loop_ids: List[int], direction: Pass_Direction, course_number: int):
        ...
        if self.object_type == 'sheet':
            ...
            is_bind_off = False
            for loop_id, target_needle in loop_id_to_target_needle.items():
                self.loop_id_to_needle_pos[loop_id] = target_needle.position
                # print(f'loop_id is {loop_id}, bind_off_loops is {bind_off_loops}, {loop_id in bind_off_loops}')
                if (loop_id in bind_off_loops) == True:
                    is_bind_off = True  
                carrier_set = self.loop_id_to_carrier[loop_id] 
            ...
            '''
                ...
                if loop_id in split_offsets:
                    ...
                    if target_needle.is_front == True:
                        knit_and_split_data_on_front[target_needle] = (Instruction_Parameters(target_needle, involved_loop=loop_id, needle_2 = split_needle, carrier=carrier_set), Instruction_Type.Split)
                    elif target_needle.is_front == False:
                        knit_and_split_data_on_back[target_needle] = (Instruction_Parameters(target_needle, involved_loop=loop_id, needle_2 = split_needle, carrier=carrier_set), Instruction_Type.Split)
                else:
                    if target_needle.is_front == True:
                        knit_and_split_data_on_front[target_needle] = (Instruction_Parameters(target_needle, involved_loop=loop_id, carrier=carrier_set), Instruction_Type.Knit)
                    elif target_needle.is_front == False:
                        knit_and_split_data_on_back[target_needle] = (Instruction_Parameters(target_needle, involved_loop=loop_id, carrier=carrier_set), Instruction_Type.Knit)
            # print(f'course number is {course_number}, loop_id_to_target_needle is {loop_id_to_target_needle}, pass direction for this knit is {current_direction}')
            if len(knit_and_split_data_on_front) != 0:
                if len(knit_and_split_data_on_front) >= 2:
                    # print(f'knit_and_split_data_on_front is {knit_and_split_data_on_front}')
                    print(f'[*knit_and_split_data_on_front.keys()] is {[*knit_and_split_data_on_front.keys()]}')
                    if [*knit_and_split_data_on_front.keys()][0].position > [*knit_and_split_data_on_front.keys()][1].position:
                        pass_direction = Pass_Direction.Right_to_Left
                    elif [*knit_and_split_data_on_front.keys()][0].position < [*knit_and_split_data_on_front.keys()][1].position:
                        pass_direction = Pass_Direction.Left_to_Right
                    carriage_pass = Carriage_Pass(pass_direction, knit_and_split_data_on_front, self._machine_state)
                elif len(knit_and_split_data_on_front) == 1: # bind-off
                    # carriage_pass = Carriage_Pass(Pass_Direction.Left_to_Right, knit_and_split_data_on_front, self._machine_state)
                    carriage_pass = Carriage_Pass(Pass_Direction.Right_to_Left, knit_and_split_data_on_front, self._machine_state)
                self._add_carriage_pass(carriage_pass, f"Knit & Split course on front {course_number}")
            if len(knit_and_split_data_on_back) != 0:
                if len(knit_and_split_data_on_back) >= 2:
                    print(f'[*knit_and_split_data_on_back.keys()] is {[*knit_and_split_data_on_back.keys()]}')
                    if [*knit_and_split_data_on_back.keys()][0].position > [*knit_and_split_data_on_back.keys()][1].position:
                        pass_direction = Pass_Direction.Right_to_Left
                    elif [*knit_and_split_data_on_back.keys()][0].position < [*knit_and_split_data_on_back.keys()][1].position:
                        pass_direction = Pass_Direction.Left_to_Right
                    carriage_pass = Carriage_Pass(pass_direction, knit_and_split_data_on_back, self._machine_state)
                elif len(knit_and_split_data_on_back) == 1: # bind-off
                    # carriage_pass = Carriage_Pass(Pass_Direction.Left_to_Right, knit_and_split_data_on_back, self._machine_state)
                    carriage_pass = Carriage_Pass(Pass_Direction.Right_to_Left, knit_and_split_data_on_back, self._machine_state)
                self._add_carriage_pass(carriage_pass, f"Knit & Split course on back {course_number}")
            # print('knit_and_split_data', knit_and_split_data)
            '''         
            return None
        
    def _cast_on(self, carrier_set:Yarn_Carrier, pass_direction: Optional[Pass_Direction] = None):
        """
        ...
        if self.object_type == 'sheet':
            if carrier_set == first_carrier_id_in_use:
                ...
                if pass_direction == Pass_Direction.Right_to_Left:
                    needles_to_tuck_on = [*range(len(first_course_loops)+10, len(first_course_loops)+15)]
                else:
                    needles_to_tuck_on = [*range(-15, -10)]
            ...
            even_pass = Carriage_Pass(Pass_Direction.Right_to_Left, even_tucks_data,
                                    self._machine_state)
            # even is actually odd because in our Needle Class, the position is added by 1 
            self._add_carriage_pass(even_pass, f"odd cast-on for {carrier_set}")
            # self._add_carriage_pass(even_pass)
            odd_pass = Carriage_Pass(Pass_Direction.Left_to_Right, odd_tucks_data,
                                    self._machine_state)
            self._add_carriage_pass(odd_pass, f"even cast-on for {carrier_set}")
            # self._add_carriage_pass(odd_pass)
            reverse_knits: Dict[Needle, Tuple[Instruction_Parameters, Instruction_Type]] = {}
            first_loops: Dict[Needle, Tuple[Instruction_Parameters, Instruction_Type]] = {}
            if carrier_set == first_carrier_id_in_use:
                # "for needle_pos, loop_id in enumerate(first_course_loops):" is not applicable for gauging that is not full gauge
        ...
        # for tube case
            ...
            for loop in first_course_loops:
                # print(f'self._knit_graph.graph.successors(loop) is {self._knit_graph.graph.successors(loop)}')
                assert self._knit_graph.graph.successors(loop) is not None, f'loop id {loop} on first course has no successor'
                successors = self._knit_graph.graph.successors(loop)
                for successor in successors:
                    if self._knit_graph.graph[loop][successor]["pull_direction"] == Pull_Direction.FtB:
                        back_bed_loops.append(loop)
                        break
                    elif self._knit_graph.graph[loop][successor]["pull_direction"] == Pull_Direction.BtF:
                        front_bed_loops.append(loop)
                        break
            # then, get every other loops on back bed and front bed to perform alternating cast-on
            first_front_tuck_loops = front_bed_loops[::2]
            second_front_tuck_loops = list(set(front_bed_loops) - set(first_front_tuck_loops))
            first_back_tuck_loops = back_bed_loops[::2]
            second_back_tuck_loops = list(set(back_bed_loops) - set(first_back_tuck_loops))
            max_wale_sum_on_front_bed = self.node_to_course_and_wale[0][1] + self.node_to_course_and_wale[back_bed_loops[-1]][1]
            if carrier_set == first_carrier_id_in_use:
                self._machine_state.last_carriage_direction = Pass_Direction.Right_to_Left
                needles_to_tuck_on_front_bed_first = []
                needles_to_tuck_on_front_bed_second = []
                needles_to_tuck_on_back_bed_first = []
                needles_to_tuck_on_back_bed_second = []
                for loop in first_front_tuck_loops:
                    wale_id = self.node_to_course_and_wale[loop][1]
                    needles_to_tuck_on_front_bed_first.append(wale_id)
                for loop in second_front_tuck_loops:
                    wale_id = self.node_to_course_and_wale[loop][1]
                    needles_to_tuck_on_front_bed_second.append(wale_id)
                for loop in first_back_tuck_loops:
                    wale_id = self.node_to_course_and_wale[loop][1]
                    needles_to_tuck_on_back_bed_first.append(max_wale_sum_on_front_bed - wale_id + 1)
                for loop in second_back_tuck_loops:
                    wale_id = self.node_to_course_and_wale[loop][1]
                    needles_to_tuck_on_back_bed_second.append(max_wale_sum_on_front_bed - wale_id + 1)
                ...
                for needle_pos in needles_to_tuck_on_front_bed_first:
                    needle_1 = Needle(True, needle_pos)
                    first_front_tucks_data[needle_1] = (Instruction_Parameters(needle_1, involved_loop=-1, carrier=carrier_set), Instruction_Type.Tuck)  # note, fake loop_id
                for needle_pos in needles_to_tuck_on_back_bed_first:
                    needle_1 = Needle(False, needle_pos)
                    first_back_tucks_data[needle_1] = (Instruction_Parameters(needle_1, involved_loop=-1, carrier=carrier_set), Instruction_Type.Tuck)  # note, fake loop_id
                for needle_pos in needles_to_tuck_on_front_bed_second:
                    needle_1 = Needle(True, needle_pos)
                    second_front_tucks_data[needle_1] = (Instruction_Parameters(needle_1, involved_loop=-1, carrier=carrier_set), Instruction_Type.Tuck)  # note, fake loop_id
                for needle_pos in needles_to_tuck_on_back_bed_second:
                    needle_1 = Needle(False, needle_pos)
                    second_back_tucks_data[needle_1] = (Instruction_Parameters(needle_1, involved_loop=-1, carrier=carrier_set), Instruction_Type.Tuck)  # note, fake loop_id
                # 
                first_pass_front = Carriage_Pass(Pass_Direction.Left_to_Right, first_front_tucks_data,
                                        self._machine_state)
                self._add_carriage_pass(first_pass_front, f"first front cast-on for {carrier_set}")
                # 
                first_pass_back = Carriage_Pass(Pass_Direction.Right_to_Left, first_back_tucks_data,
                                        self._machine_state)
                self._add_carriage_pass(first_pass_back, f"first back cast-on for {carrier_set}")
                # 
                second_pass_front = Carriage_Pass(Pass_Direction.Left_to_Right, second_front_tucks_data,
                                        self._machine_state)
                self._add_carriage_pass(second_pass_front, f"second front cast-on for {carrier_set}")
                # 
                second_pass_back = Carriage_Pass(Pass_Direction.Right_to_Left, second_back_tucks_data,
                                        self._machine_state)
                self._add_carriage_pass(second_pass_back, f"second back cast-on for {carrier_set}")
                first_loops_front: Dict[Needle, Tuple[Instruction_Parameters, Instruction_Type]] = {}
                first_loops_back: Dict[Needle, Tuple[Instruction_Parameters, Instruction_Type]] = {}
                for needle_pos, loop_id in zip(sorted(needles_to_tuck_on_front_bed_first + needles_to_tuck_on_front_bed_second), front_bed_loops):
                    needle_1 = Needle(True, needle_pos)
                    self.loop_id_to_needle_pos[loop_id] = needle_pos #has to be placed before instruction because we need it in instruction
                    first_loops_front[needle_1] = (Instruction_Parameters(needle_1, involved_loop=loop_id, carrier=carrier_set), Instruction_Type.Knit)  
                for needle_pos, loop_id in zip(sorted(needles_to_tuck_on_back_bed_first + needles_to_tuck_on_back_bed_second, reverse= True), back_bed_loops):
                    needle_1 = Needle(False, needle_pos)
                    self.loop_id_to_needle_pos[loop_id] = needle_pos
                    first_loops_back[needle_1] = (Instruction_Parameters(needle_1, involved_loop=loop_id, carrier=carrier_set), Instruction_Type.Knit)  
                carriage_pass = Carriage_Pass(Pass_Direction.Left_to_Right, first_loops_front, self._machine_state)
                self._add_carriage_pass(carriage_pass, f"first row loops on front bed for for {carrier_set}")
                carriage_pass = Carriage_Pass(Pass_Direction.Right_to_Left, first_loops_back, self._machine_state)
                self._add_carriage_pass(carriage_pass, f"first row loops on back bed for {carrier_set}")
            else:
                # for other carriers, tuck on needles on the side to avoid messing the designed pattern, but note that tuck on which side is determined by the pass direction,
                # if ignore this, introducing a new yarn could fail
                needles_to_tuck_on = []
                if pass_direction == Pass_Direction.Right_to_Left:
                    needles_to_tuck_on = [*range(int(len(first_course_loops)/2)*self.wale_dist+10, int(len(first_course_loops)/2)*self.wale_dist+15)]
                else:
                    needles_to_tuck_on = [*range(-15, -10)]
                for needle_pos in needles_to_tuck_on:
                    needle_1 = Needle(True, needle_pos)
                ...
                self._add_carriage_pass(carriage_pass, f"stabilize cast-on for {carrier_set}")
                carriage_pass = Carriage_Pass(Pass_Direction.Left_to_Right, first_loops, self._machine_state)
                self._add_carriage_pass(carriage_pass, f"first row loops for {carrier_set}")
