...
# 66th round [p, k] to end.
    ...
    # knit_graph = compiler.compile(56, 21, object_type = 'tube', pattern = tube_pattern) #[12, 10]; for tube [12, 5]; [48, 30]
    ...
    # pocket_generator = Pocket_Generator_on_Sheet(parent_knitgraph = knit_graph, sheet_yarn_carrier_id = sheet_yarn_carrier_id, pocket_yarn_carrier_id=4, is_front_patch = False, left_keynodes_child_fabric=[(3, 30), (6, 30)], right_keynodes_child_fabric=[(3, 22), (7, 9)], close_top = False, edge_connection_left_side = [True, False, False], edge_connection_right_side = [True])
    ...
    # pocket_generator = Pocket_Generator_on_Tube(parent_knitgraph = knit_graph, tube_yarn_carrier_id = tube_yarn_carrier_id, pocket_yarn_carrier_id = 4, is_front_patch = False, left_keynodes_child_fabric=[(3, 4), (6, 4)], right_keynodes_child_fabric=[(3, 10), (6, 10)], edge_connection_left_side = [True], close_top = False, edge_connection_right_side = [True])
    # knitGraph = pocket_generator.build_pocket_graph() 
    # KnitGraph_Visualizer = knitGraph_visualizer(knitGraph)
    # KnitGraph_Visualizer.visualize()
    ...
    # handle_generator = Handle_Generator_on_Sheet(parent_knitgraph = knit_graph, sheet_yarn_carrier_id = sheet_yarn_carrier_id, handle_yarn_carrier_id = 4, is_front_patch = False, left_keynodes_child_fabric=[(10, 9), (20, 9)], right_keynodes_child_fabric=[(10, 41), (20, 41)])
    ...
    # handle_generator = Handle_Generator_on_Tube(parent_knitgraph = knit_graph, tube_yarn_carrier_id = tube_yarn_carrier_id, handle_yarn_carrier_id=4, is_front_patch = False, left_keynodes_child_fabric=[(5, 10), (15, 10)], right_keynodes_child_fabric=[(5, 40), (15, 40)])
    # knitGraph = handle_generator.build_handle_graph() 
    # KnitGraph_Visualizer = knitGraph_visualizer(knitGraph)
    # KnitGraph_Visualizer.visualize()
...
def test_lace():
    ...
    # hole_generator = Hole_Generator_on_Sheet(yarns_and_holes_to_add = {2:[38]}, knitgraph = knit_graph)
    # knitGraph = hole_generator.add_hole()
    # # note that we only update (delete hole nodes on the self._knit_graph, we do not correspondingly update nodes in both self.node_on_front_or_back and self.node_to_course_and_wale)
    # KnitGraph_Visualizer = knitGraph_visualizer(knitGraph)
    # KnitGraph_Visualizer.visualize()
